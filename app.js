...
let quiz = [
  ...
  { 
    ...
    question:'第29条によれば、運転者は道路標識等により「　」が禁止された道路で駐車・停車してはならない。',
    answers: ['駐車及び停車','右折及び左折','乗降','進行'],
    correct:'駐車及び停車'
  },
  { 
    question:'第30条によれば、運転者は「　」を装着しないで自動車を運転してはならない。',
    answers: ['ヘルメット','シートベルト','制服','名札'],
    correct:'シートベルト'
  },
  { 
    question:'第31条によれば、原則として「　」を使用してはならない。ただし、危険を防止する場合を除く。',
    answers: ['警音器','方向指示器','ヘッドライト','ワイパー'],
    correct:'警音器'
  },
  { 
    question:'第32条によれば、歩行者や自転車の通行を妨げるおそれがあるときは、自動車を横断・回転又は「　」させてはならない。',
    answers: ['発進','後退','停車','加速'],
    correct:'後退'
  },
  { 
    question:'第32条によれば、横断歩道等に歩行者がいるときは「　」し、その通行を妨げてはならない。',
    answers: ['一時停止','減速','加速','方向転換'],
    correct:'一時停止'
  },
  { 
    question:'第33条によれば、運転者は停留所に接近するときは「　」で停止できる速度で進行しなければならない。',
    answers: ['安全距離','停留所','乗客','停車'],
    correct:'停留所'
  },
  { 
    question:'第34条によれば、停留所に利用客がいて満員のときは、一旦停車し「　」でその旨を告げて通過できる。',
    answers: ['車外マイク','点呼簿','乗車券','標識灯'],
    correct:'車外マイク'
  },
  { 
    question:'第37条によれば、原則として停留所以外での乗降はしてはならない。ただし「　」区間では一定条件下で可能である。',
    answers: ['フリー乗降','安全確認','徐行運転','回送運転'],
    correct:'フリー乗降'
  },
  { 
    question:'第39条によれば、運転者は乗客の動向を注視し「　」に努めなければならない。',
    answers: ['車内事故防止','車両整備','運転技術向上','交通渋滞解消'],
    correct:'車内事故防止'
  },
  { 
    question:'第40条によれば、扉を開閉するときは「　」等で乗客の安全を確認しなければならない。',
    answers: ['ミラーやパイロットランプ','方向指示器','タイヤ','運転免許証'],
    correct:'ミラーやパイロットランプ'
  }, { 
    question:'第41条によれば、運転者は乗客の「　」を確認した後でなければ自動車を発進してはならない。',
    answers: ['安全','乗車券','人数','行き先'],
    correct:'安全'
  },
  { 
    question:'第42条によれば、勾配箇所など安全な運行に支障のおそれがある箇所を通過するときは「　」を降車させなければならない。',
    answers: ['乗客','運行管理者','添乗員','整備士'],
    correct:'乗客'
  },
  { 
    question:'第44条によれば、下り勾配箇所ではエンジンブレーキと「　」を併用しなければならない。',
    answers: ['フットブレーキ','サイドブレーキ','駐車ブレーキ','補助ブレーキ'],
    correct:'フットブレーキ'
  },
  { 
    question:'第45条によれば、上り勾配で発進するときは原則として「　」を使用しなければならない。',
    answers: ['駐車ブレーキ','フットブレーキ','方向指示器','クラッチ'],
    correct:'駐車ブレーキ'
  },
  { 
    question:'第46条によれば、登坂箇所で行き違う場合は「　」勾配の車両が一時停車し、登り坂の車両を優先させる。',
    answers: ['下り','上り','緩やかな','急な'],
    correct:'下り'
  },
  { 
    question:'第47条によれば、見通しの悪いカーブを通過するときは必ず「　」しなければならない。',
    answers: ['徐行','加速','停止','追い越し'],
    correct:'徐行'
  },
  { 
    question:'第52条によれば、踏切を通過するときは直前で「　」し、安全を確認した後でなければ進行してはならない。',
    answers: ['一時停止','徐行','加速','停車禁止'],
    correct:'一時停止'
  },
  { 
    question:'第53条によれば、踏切が遮断されているときや「　」が作動しているときは進入してはならない。',
    answers: ['警報装置','ヘッドライト','速度計','方向指示器'],
    correct:'警報装置'
  },
  { 
    question:'第54条によれば、踏切内で運行不能になった場合、運転者は直ちに全ての「　」を開放し、乗客を退避させなければならない。',
    answers: ['扉','窓','座席','ハッチ'],
    correct:'扉'
  },
  { 
    question:'第55条によれば、事故発生時に最優先される措置は「　」である。',
    answers: ['負傷者の救護','車両番号の確認','営業所への報告','乗客への謝罪'],
    correct:'負傷者の救護'
  },{ 
    question:'第56条によれば、運転者は運行中に自動車の故障を発見したときは直ちに停車し「　」に報告しなければならない。',
    answers: ['運行管理者','整備士','営業所長','乗客'],
    correct:'運行管理者'
  },
  { 
    question:'第57条によれば、自動車専用道で故障した場合、必ず「　」を表示し、赤旗を掲げ、ハザードランプを点滅させなければならない。',
    answers: ['停止表示板','点呼簿','車検証','乗務記録簿'],
    correct:'停止表示板'
  },{ 
    question:'第59条によれば、運転者は常に「　」を的確に把握し、被害を未然に防止するよう努めなければならない。',
    answers: ['気象状況','乗車人数','車両番号','交通規制'],
    correct:'気象状況'
  },
  { 
    question:'第61条によれば、地震や台風で運行継続が危険なときは直ちに運行を中断し「　」に報告しなければならない。',
    answers: ['運行管理者','警察署','整備士','消防署'],
    correct:'運行管理者'
  },
  { 
    question:'第62条によれば、著しい暴風のときは運転を一時見合わせ「　」に報告し指示を受けなければならない。',
    answers: ['運行管理者','乗客代表','営業所長','整備士'],
    correct:'運行管理者'
  }, { 
    question:'第63条によれば、濃霧時には速度を減速し、前照灯のほか「　」を点灯して運転しなければならない。',
    answers: ['フォグランプ','ハザードランプ','ウインカー','車内灯'],
    correct:'フォグランプ'
  },{ 
    question:'第64条によれば、降雪や凍結時には「　」を装着し、速度と制動に十分注意して運転しなければならない。',
    answers: ['タイヤチェーン','スノーボード','滑り止め砂','ABS装置'],
    correct:'タイヤチェーン'
  }, { 
    question:'第66条によれば、自動車に火災が発生した場合は直ちに停車し、すべての「　」を開け、乗客を避難誘導しなければならない。',
    answers: ['扉','窓','座席','荷物室'],
    correct:'扉'
  }, { 
    question:'第67条によれば、運転者は常に「　」の備付け位置を確認し、その使用方法を熟知しておかなければならない。',
    answers: ['消火器','非常口','発煙筒','停止表示板'],
    correct:'消火器'
  },
  { 
    question:'第68条によれば、運転者は「　」かつ公平な接客に努めなければならない。',
    answers: ['親切・丁寧','迅速・効率的','静粛・迅速','強制・簡潔'],
    correct:'親切・丁寧'
  },{ 
    question:'第69条によれば、運転者は「　」による案内放送を適切な音量で行わなければならない。',
    answers: ['音声合成放送装置','ICカード端末','方向指示器','運行指示表'],
    correct:'音声合成放送装置'
  },{ 
    question:'第69条によれば、運転者は「　」を正しく装着し、必要な案内を丁寧に行わなければならない。',
    answers: ['マイク','制服','名札','イヤホン'],
    correct:'マイク'
  },{ 
    question:'第71条によれば、運転者は「　」に直接手を触れてはならない。ただし乗客が投入できない場合を除く。',
    answers: ['現金','整理券','ICカード','運賃箱'],
    correct:'現金'
  },
  { 
    question:'第72条によれば、終点では必ず「　」及び乗客の有無を確認しなければならない。',
    answers: ['車内の遺失物','車両番号','燃料残量','アルコール検査結果'],
    correct:'車内の遺失物'
  },
  { 
    question:'第72条によれば、遺失物を発見したときは「　」に記入し、運行管理者に提出しなければならない。',
    answers: ['遺失物整理票','車内得物お預り票','勤務票','事故報告書'],
    correct:'遺失物整理票'
  },{ 
    question:'第74条によれば、車内で「　」をすることは禁止されている。',
    answers: ['喫煙','読書','睡眠','会話'],
    correct:'喫煙'
  }
];
